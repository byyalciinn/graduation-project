...
import { Search, MoreVertical, ChevronLeft, ChevronRight, ShoppingBag } from "lucide-react"
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold tracking-tight text-[#1F1B24]">Buyers</h1>
            </div>
        ...
        <Card className="border-gray-200 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-gray-100 bg-gray-50/50">
            <div>
              <CardTitle className="text-xl font-semibold text-[#1F1B24]">All Buyers</CardTitle>
              <CardDescription className="text-gray-600">
                {loading ? "Loading..." : `Showing ${users.length} of ${pagination.totalCount} buyers`}
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
              ...
              <>
                <div className="space-y-4">
                  {users.map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center justify-between p-5 rounded-xl border border-gray-200 hover:border-emerald-500/30 hover:shadow-md transition-all duration-200 bg-white"
                    >
                      <div className="flex items-center gap-4 flex-1">
                        <Avatar className="h-12 w-12 ring-2 ring-emerald-100">
                          <AvatarImage src={user.image || undefined} alt={user.name || "Buyer"} />
